...
class LocationConverter {
  ...
  func convertGrid(lon: Double, lat: Double) -> (Int, Int) {
    ...
    let gridX: Double = ra * sin(theta) + map.xo
    let gridY: Double = ro - ra * cos(theta) + map.yo
    
    return (Int(gridX + 1.5), Int(gridY + 1.5))
  }
  
  func convertAddress(lon: Double, lat: Double) -> Observable<(String, String)> {
    return Observable.create { observer in
      ...
      geoCoder.reverseGeocodeLocation(findLocation, preferredLocale: local) { (place, error) in
        if let address: [CLPlacemark] = place {
          iPrint(address.last?.locality)
          si = address.last!.administrativeArea!
          gu = address.last!.locality!
          
          observer.onNext((si, gu))
